...
function EditPanelMerek () {
    ...
    const { id } = useParams();  // Extract the ID from the UR

    const [brand, setBrand] = useState([]);
    useEffect(() => {
        const fetchData = async () => {
            try{
                const response = await apiAuth.put('/brand/update')
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await apiAuth.put('/brand/update', formData);
            console.log(response.data.data[0].insertId);
            textPopUp("Success", "Berhasil memperbarui data", "success");
        } catch (err) {
            console.error('Error uploading files:', err);
            textPopUp("Error", "Gagal memperbarui data", "error");
        }
    };
                            ...
                            <div className="relative flex flex-col border border-dashed bg-clip-border rounded-2xl">
                                <div className="px-9 pt-5 flex justify-between items-stretch flex-wrap pb-0 bg-transparent">
                                    <h3 className="flex flex-col items-start justify-center ml-0 font-medium">
                                        <span className="mr-3 font-semibold">Upate Data</span>
